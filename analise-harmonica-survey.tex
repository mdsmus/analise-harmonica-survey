...
Representing musical information on a computer is not a solved
problem, there being a multitude of possible codifications, from more
sound-oriented ones - such as MIDI and piano roll notation - to
notations, such as \cite{kroger06:processo}, specifically made to
better transmit harmonic information.

Interestingly, the choice of a proper codification influences the
results obtainable from the later analysis. For example, a
representation which fails to distinguish enharmonic pitches might
make harder to recognize a neapolitan sixth from an augmented seventh.
Thus, it is crucial that a good input format be chosen so as to
minimize errors and necessary human input in the analysis.
...
segments with chords and tonal functions. The segmentation problem
isn't clearly formalized in musical theory texts, complicating the
task of designing an effective algorithmical solution for it. We
believe that a more systematic approach to music segmentation might
wield considerably better results, since the segmentation problem for
harmonic analysis of tonal music is largely unsolved. 

This problem is non-trivial, since the number of possible partition
points in a given piece is proportional to the number of notes -
usually divided by a constant representing the number of voices and
likelihood of chords and double-steps - and, so, the number of
possible segmentations is approximately two to the power of the number
of notes in a score.

Due to its high computational complexity - the number of
