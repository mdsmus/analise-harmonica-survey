...
automated harmonic analysis has been approached, from several
different directions, with limited success. Expert systems and
statistical approaches were tried.

Little has been done, however, towards stabilishing standard
benchmarks. The problem of harmonic-significant segmentation is
unsolved. A more systematic approach to its solution might resolve
ambiguities in existent methods.  Most algorithms also ignore relevant
contextual clues. A proper formalisation of musical theory would
facilitate the construction of effective analysis systems.

Here we present a comprehensive survey of the existing literature on
the subject, as a means to facilitate more focused research on the
area.
\end{abstract}
...
survey on the topic. We chose to ignore audio processing techniques
and focus solely on extraction of harmony from a score.

\nota{falar como serão feitas as coisas nesse artigo}
\nota{falar que não vai falar de áudio}
...
effectiveness. We start with an overview of the problem of harmonic
analysis, then a clarification of what we mean by harmonic
context. The problem of representing music in a form amenable to
analysis is observed next. We follow with a discussion of the existent
algorithms for chord and key detection, techniques for segmentation
and existent benchmarks.
...
the same direction. Some algorithms ignore or make minimal use of the
harmonic context. Proper use of harmonic context helps to solve
...
harmony rules.
Pardo et al. \cite{pardo99:automated} classify error frequencies.
Most of them are related to harmonic context, specifically non-chord
notes and cadence recognition. They agree that a deeper understanding
of voice leading and knowledge about neighbour chords would help
adjusting chord-label weights.
...
represent notes as pitch classes, ignoring enarmonic information. This
is somewhat emphasized by the fact that MIDI or MIDI-like input is
used \cite{pardo99:automated,wang97:generating,raphael03:harmonic}.
Pardo \cite{pardo02:algorithms,pardo99:automated} not only uses only
pitch classes, but ignores meter and all forms of context information,
with the intention of providing a basis for study of better
algorithms.
...
analysing bach chorales with over 90\% efficiency. A hidden markov
...
is split if it spans more than a bar. Barthèlemy
...
models such as set theory (of post-tonal theory), it is not clear in
literature if a mathematical approach would ease harmonic
analysis. Taneyev \cite{taneyev62:counterpoint} has used mathematics
extensively in his counterpoint treatise with a good degree of
success. As far as we know, there are not meaningful works in the
literature of automatic harmonic analysis where a mathematical model
such as set theory is used extensively.
...
\nota{<passos> isso é realmente relevante para a conclusão?}
...
\nota{para passos: não consigo ver aonde}
Barthèlemy \cite{barthelemy01:figured} claims to distinguish
