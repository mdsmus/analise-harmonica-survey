...
system because some chords and notes (non-chord notes) can be
understood only if this context is considered. The harmonic context
helps to solve problems as ambiguities in chord analysis and
identification of non-chord notes and suspension and ritardo
resolutions.

Most of systems for automated harmonic analysis don't consider the
harmonic context in their processing
\cite{harte.sandler.ea:symbolic,pardo99:automated,pardo02:algorithms}.
Errors in chord analysis can be solved considering harmonic context.
...
the answer errors are, in most of cases related to context.

Some of these research projects consider harmonic context, as the NUSO
system \cite{mouton95:numeric}, that uses harmonic context to define
tonality through rules that consider non-chord notes in accompanied
melody, Alberti bass and baroque style; the MusES system that uses
relations between adjacent chords to define the best possible tonality
for a chord; and the automated harmonic analysis method developed by
\cite{barthelemy01:figured} that has a simple algorithm to process
...
\cite{pardo02:algorithms} before his paper. And not many have
published this sort of data after him. The lack of this data in the
literature makes dificult to compare different systems.

There is not a specific framework for comparison among different
...
other authors. Pardo compares his results with
...
comparision whe know of. However, these comparisons were based on the
results published in papres and not in a direct implementation. This
means that only the examples analysed by the authors can be compared.

We believe that it is important to have standard tests in colections
of examples. As stated in the section \ref{sec:introduction} of this
paper, we believe that having a large corpus of music is necessary to
the success of testing a specific algorithm. Pickens
\cite{pickens02:polyphonic} not only agrees that is necessary to have
standard tests as he uses a large corpus of music in his research.

Ele usa 45 trechos musicais do Kostka e Payne \cite{kostka03:tonal}
(compilados por Temperley). Alguns desses materiais, assim como
